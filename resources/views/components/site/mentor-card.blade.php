...
<article {{ $attributes->merge(['class' => 'group rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-[#292b86]/30 hover:shadow-xl hover:shadow-[#292b86]/10']) }}>
    <a href="{{ route('mentors.show', $routeKey) }}" class="block">
        <div class="relative mx-auto h-28 w-28 overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#292b86] via-[#f15a24] to-[#ed1c24] p-1 shadow-lg shadow-slate-200">
            <div class="h-full w-full overflow-hidden rounded-[1.7rem] bg-white">
                @if($imageUrl)
                    <img src="{{ $imageUrl }}" alt="{{ $name }}" class="h-full w-full object-cover transition duration-500 group-hover:scale-105">
                @else
                    <div class="flex h-full w-full items-center justify-center bg-[#292b86]/10 text-2xl font-black text-[#292b86]">{{ $initials ?: 'M' }}</div>
                @endif
            </div>
        </div>
    </a>
